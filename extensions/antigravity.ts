...
import { calculateCost, createAssistantMessageEventStream, type AssistantMessage, type Api } from "@earendil-works/pi-ai";
    ...
    try {
      ...
      if (!conversationId) {
        try {
          ...
          if (error instanceof Error && error.name === "TimeoutError") throw new Error("agy timed out");
          throw error;
        }
      ...
      const handle = runtime.pool.acquire(rawSessionId, cwd, resolved.model, resolved.effort, conversationId ?? undefined);
      ...
      if (!conversationId && before) conversationId = await findNewConversation(before, runtime.config.conversationsDir);
      ...
      if (textStarted && textIndex !== null) {
        ...
        if (block?.type === "text" && Buffer.byteLength(block.text, "utf8") > fileThreshold) {
          ...
          const outPath = join(outDir, `${poolKey.replace(/[^a-zA-Z0-9_-]/g, "_")}-${Date.now()}.md`);
    ...
    createProvider({
      ...
      auth: {
        apiKey: {
          ...
          resolve: async () => ({ auth: { apiKey: "antigravity" }, source: "Antigravity CLI" }),
        },
      },
      models: poolModels,
