...
import { CodexAcpClient } from "./client.ts";
import { resolveCodexConfig, type CodexModelMeta } from "./models.ts";
...
export interface CodexStreamRuntime {
  config: CodexConfig;
  getCwd: () => string;
  getMeta: (modelId: string) => CodexModelMeta | undefined;
  store: SessionStore;
  ...
  (async () => {
    ...
    try {
      ...
      const remainingTimeout = () => {
        ...
        if (remaining <= 0) throw new Error("codex-acp timed out");
        return remaining;
      };
      ...
      remainingTimeout();
      if (!prompt.trim()) {
        throw new Error("codex turn has no user text");
      }
      ...
      if (result.stopReason === "cancelled") {
        ...
        if (output.stopReason === "error") {
          output.errorMessage = "codex-acp cancelled";
        }
        stream.push({
          type: output.stopReason === "aborted" ? "error" : "error",
