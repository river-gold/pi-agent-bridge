...
export class AgyPool {
	...
	private ensureEntry(
		...
		if (existing && !existing.closed) {
			...
			const modelMismatch =
				(existing.model ?? undefined) !== (model ?? undefined);
			const effortMismatch =
				(existing.effort ?? undefined) !== (effort ?? undefined);
	...
	private handleLine(entry: PoolEntry, rawLine: string) {
		...
		if (parsed.event === "step_update") {
			...
			if (
				...
				const stepIndex =
					typeof step.step_index === "number" ? step.step_index : Date.now();
	...
	private settlePending(entry: PoolEntry) {
		...
		const finalText =
			...
			pending.resultResponse ||
			(!pending.sawValidEvent ? "" : "");
		const hasAnswer = Boolean(
			pending.accumulatedText || pending.resultResponse,
		);
		...
		if (!hasAnswer) {
			if (pending.resultStatus && pending.resultStatus !== "SUCCESS") {
				pending.reject(
					new Error(
						pending.resultError?.trim() ||
							`agy failed with status ${pending.resultStatus}`,
					),
				);
				return;
			}
		...
		return new Promise<RunPooledResult>((resolve, reject) => {
			...
			try {
				...
				const ok = entry.child.stdin!.write(line, (err) => {
					if (err)
						settleReject(
							new Error(`failed to write to agy pool: ${err.message}`),
						);
				});
				if (!ok) {
