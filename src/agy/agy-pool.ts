...
export class AgyPool {
  ...
  private spawnEntry(
    ...
    child.on("error", (err) => this.onCrash(entry, new Error(`failed to spawn agy: ${err.message}`)));
    child.on("close", (code, signal) => {
      if (entry.closed) return;
      // if pending, reject; otherwise just remove
      if (entry.pending) {
        const msg = signal ? `agy pool process killed by ${signal}` : `agy pool process exited ${code ?? "unknown"}`;
  ...
  private handleLine(entry: PoolEntry, rawLine: string) {
    ...
    if (parsed.event === "step_update") {
      ...
      if (stepType === "tool" && typeof step.tool_name === "string" && pending) {
        ...
        if (state === "ACTIVE") {
          pending.onEvent?.({ type: "tool_start", id: toolId, name: toolName, args: params });
        } else if (state === "DONE") {
          pending.onEvent?.({ type: "tool_end", id: toolId, name: toolName, args: params, output });
        }
      }
      return;
    }
  ...
  private settlePending(entry: PoolEntry) {
    ...
    const finalText = pending.accumulatedText || pending.resultResponse || (!pending.sawValidEvent ? "" : "");
    ...
    if (!hasAnswer) {
      if (pending.resultStatus && pending.resultStatus !== "SUCCESS") {
        pending.reject(new Error(pending.resultError?.trim() || `agy failed with status ${pending.resultStatus}`));
        return;
      }
    ...
    pending.resolve({
      ...
      stderr,
      exitCode: 0,
      ...(pending.conversationId || entry.conversationId ? { conversationId: pending.conversationId ?? entry.conversationId } : {}),
  ...
  _exec(entry: PoolEntry, prompt: string, opts: PoolPromptOptions = {}): Promise<RunPooledResult> {
    ...
    const task: Promise<RunPooledResult> = entry.queue.then(() => this.doExec(entry, prompt, opts, timeoutMs));
