...
import { CursorAcpClient } from "./client.ts";
import { resolveCursorModel, type CursorModelMeta } from "./models.ts";
...
export interface CursorStreamRuntime {
  config: CursorConfig;
  getCwd: () => string;
  getMeta: (modelId: string) => CursorModelMeta | undefined;
  store: SessionStore;
  ...
  (async () => {
    ...
    try {
      ...
      const remainingTimeout = () => {
        ...
        if (remaining <= 0) throw new Error("cursor-acp timed out");
        return remaining;
      };
      ...
      if (result.stopReason === "cancelled") {
        ...
        if (output.stopReason === "error") {
          output.errorMessage = "cursor-acp cancelled";
        }
        stream.push({
          type: "error",
