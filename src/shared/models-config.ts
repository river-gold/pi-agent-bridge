...
async function readModelsConfigFile(
	configPath: string,
	...
	if (errors.length > 0) {
		throw new Error(
			`Invalid JSONC in models config ${configPath}: ${errors[0].error}`,
		);
	}
	return { path: configPath, config: parseModelsConfig(parsed), exists: true };
}

export async function loadModelsConfigFile(
	path?: string,
): Promise<LoadedModelsConfig> {
