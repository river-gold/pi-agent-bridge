...
describe("agy-pool (long-lived)", () => {
  it("1. Pool reuses process for same sessionKey (second turn does not respawn)", async () => {
    ...
    const pool = new AgyPool({ binary: wrapper, idleTimeoutMs: 60_000, timeoutMs: 5000 });
    try {
      ...
      assert.equal(h2.key, key, "same sessionKey should map to same composite key");
      const r2 = await h2.prompt("turn2");
      assert.equal(r2.stdout, "echo:turn2");
      assert.equal(r2.conversationId, conv1, "reused process keeps same conversationId");
  ...
  it("2. Different sessionKey (different sessionId or cwd) gets isolated processes and conversationIds", async () => {
    ...
    try {
      ...
      assert.notEqual(ra.conversationId, rb.conversationId, "different sessionId must have different conversationIds");
      ...
      assert.equal(new Set(pids).size, 3, "each isolated key should have distinct pid");
  ...
  it("4. Serial queue: concurrent turns on same key are queued, not interleaved", async () => {
    ...
    try {
      ...
      const p1 = handle.prompt("slow-first", { onEvent: (e) => { if (e.type === "text") eventsA.push(e.text); } });
      const p2 = handle.prompt("second-fast", { onEvent: (e) => { if (e.type === "text") eventsB.push(e.text); } });
      const t0 = Date.now();
      ...
      assert.ok(elapsed >= 180, `queue should serialize, elapsed ${elapsed}ms too short`);
      ...
      const order: string[] = [];
      const pa = h.prompt("slow-A").then((r) => { order.push(r.stdout); return r; });
      const pb = h.prompt("slow-B").then((r) => { order.push(r.stdout); return r; });
      const pc = h.prompt("C").then((r) => { order.push(r.stdout); return r; });
      ...
      assert.deepEqual(order, ["echo:slow-A", "echo:slow-B", "echo:C"], "queue must preserve submission order");
      const cnt = await spawnCount(spawnLog);
  ...
  it("5. Crash recovery: killed process is evicted and next turn respawns", async () => {
    ...
    try {
      ...
      assert.notEqual(r2.conversationId, conv1, "respawned process should have new conversationId");
  ...
  it("6a. AbortSignal handling aborts in-flight turn and evicts process", async () => {
    ...
    try {
      ...
      assert.ok(after >= before, `spawn count should not decrease after respawn ${before} -> ${after}`);
      // immediate abort before even writing
      const ac2 = new AbortController();
      ac2.abort();
      await assert.rejects(() => pool.acquire("abort-sess2", cwd).prompt("x", { signal: ac2.signal }), (err: Error) => {
        assert.equal(err.name, "AbortError");
        return true;
      });
    } finally {
  ...
  it("6b. timeout handling rejects and evicts, next turn respawns", async () => {
    ...
    `;
    const wrapper = await makeMockPair(root, spawnLog, handler);
    const pool = new AgyPool({ binary: wrapper, idleTimeoutMs: 60_000, timeoutMs: 150 });
    try {
      await assert.rejects(() => pool.acquire("to-sess", cwd).prompt("timeout-me", { timeoutMs: 120 }), (err: Error) => {
      ...
      const after = await spawnCount(spawnLog);
      assert.ok(after >= before, `spawn count should not decrease ${before} -> ${after}`);
  ...
  it("7a. idle eviction removes unused entries", async () => {
    ...
    await writeFile(spawnLog, "", "utf-8");
    const wrapper = await makeMockPair(root, spawnLog, echoHandler);
    const pool = new AgyPool({ binary: wrapper, idleTimeoutMs: 900, timeoutMs: 5000, maxEntries: 20 });
    try {
  ...
  it("7b. graceful shutdown disposeAll kills processes and clears pool", async () => {
    ...
    try {
      ...
      const after = await spawnCount(spawnLog);
      assert.equal(after, before + 1, "after disposeAll next acquire should respawn");
    } finally {
      await pool.disposeAll().catch(() => {});
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("8. Delta/streaming: text_deltas are forwarded in order", async () => {
    ...
    try {
      ...
      assert.deepEqual(deltas, ["hello ", "world", "! delta"], "deltas must be forwarded in order");
      assert.equal(r.stdout, "hello world! delta");
      assert.equal(deltas.join(""), r.stdout);
      // ensure conversation event also forwarded
      assert.ok(events.some((e: unknown) => (e as { type: string }).type === "conversation"));
      // ensure tool events forwarded
      assert.ok(events.some((e: unknown) => (e as { type: string }).type === "tool_start"));
      assert.ok(events.some((e: unknown) => (e as { type: string }).type === "tool_end"));
    } finally {
      await pool.disposeAll().catch(() => {});
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("9. Spawn error handling: missing binary rejects and does not leave stray entry", async () => {
    ...
    const pool = new AgyPool({ binary: bogusBinary, idleTimeoutMs: 60_000, timeoutMs: 1200 });
    try {
      await assert.rejects(() => pool.acquire("err-key", cwd).prompt("hi"), (err: Error) => {
      ...
      await assert.rejects(() => pool.acquire("err-key2", cwd).prompt("hi2"), (err: Error) => {
        assert.match(err.message, /failed to spawn agy|ENOENT|spawn/i);
        return true;
      });
    } finally {
    ...
    await writeFile(badWrap, `#!/usr/bin/env bash\nexec ${process.execPath} ${JSON.stringify(badJs)} "$@"\n`, "utf-8");
    await chmod(badJs, 0o755);
    await chmod(badWrap, 0o755);
    const pool2 = new AgyPool({ binary: badWrap, idleTimeoutMs: 60_000, timeoutMs: 1200 });
    try {
      await assert.rejects(() => pool2.acquire("bad-key", cwd2).prompt("hi"), (err: Error) => {
  ...
  it("pool respects maxEntries LRU eviction and does not exceed limit", async () => {
    ...
    await writeFile(spawnLog, "", "utf-8");
    const wrapper = await makeMockPair(root, spawnLog, echoHandler);
    const pool = new AgyPool({ binary: wrapper, idleTimeoutMs: 60_000, timeoutMs: 5000, maxEntries: 2 });
    try {
  ...
  it("spawn args include --input-format stream-json, --output-format stream-json, --add-dir and --dangerously-skip-permissions", async () => {
    ...
    try {
      ...
      assert.ok(args.includes("--dangerously-skip-permissions"), "missing --dangerously-skip-permissions");
      const inputIdx = args.indexOf("--input-format");
      assert.equal(args[inputIdx + 1], "stream-json", "--input-format must be stream-json");
      const outIdx = args.indexOf("--output-format");
      assert.equal(args[outIdx + 1], "stream-json", "--output-format must be stream-json");
