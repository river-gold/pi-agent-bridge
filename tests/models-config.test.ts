...
type ChildLoadResult =
  | { ok: true; loaded: { path: string; config: ModelsConfigFile; exists: boolean } }
...
function assertLoaded(result: ChildLoadResult): asserts result is Extract<ChildLoadResult, { ok: true }> {
...
describe("models-config", () => {
  ...
  it("loads explicit, env, project, then home without cross-file merging", async () => {
    ...
    try {
      ...
      const explicit = await loadModelsConfigInChild({ cwd: project, home, explicitPath, envPath });
      assertLoaded(explicit);
      assert.equal(explicit.loaded.path, explicitPath);
      assert.deepEqual(Object.keys(explicit.loaded.config.agy?.models ?? {}), ["explicit"]);
      ...
      assert.deepEqual(explicitMissing.loaded, { path: missingExplicit, config: {}, exists: false });

      const fromEnv = await loadModelsConfigInChild({ cwd: project, home, envPath });
      assertLoaded(fromEnv);
      assert.equal(fromEnv.loaded.path, envPath);
      assert.deepEqual(Object.keys(fromEnv.loaded.config.agy?.models ?? {}), ["env"]);

      const missingEnv = join(root, "missing-env.jsonc");
      const envMissing = await loadModelsConfigInChild({ cwd: project, home, envPath: missingEnv });
      assertLoaded(envMissing);
      assert.deepEqual(envMissing.loaded, { path: missingEnv, config: {}, exists: false });
      ...
      assert.deepEqual(emptyProject.loaded, { path: projectPath, config: {}, exists: true });
      ...
      assert.deepEqual(Object.keys(fromHome.loaded.config.agy?.models ?? {}), ["home"]);

      await rm(homePath);
      const bothMissing = await loadModelsConfigInChild({ cwd: project, home });
      assertLoaded(bothMissing);
      assert.deepEqual(bothMissing.loaded, { path: projectPath, config: {}, exists: false });
    } finally {
  ...
  it("does not fall back to home for malformed or non-ENOENT project errors", async () => {
    ...
    try {
      ...
      if (malformed.ok) assert.fail("expected malformed project config to fail");
...
`,
      ...
      const agy = await discoverAgy({ configPath: path });
      assert.deepEqual(agy.models.map((model) => model.id), ["jsonc-agy"]);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
