...
`,
  ...
  it("keeps streamed answer when agy fails late on toolSummary/toolAction", async () => {
    const env = await createE2EEnv();
    try {
      await writeMockAgy(
        env,
        `
emit([
  { event: "init", conversation_id: "c-late" },
  { event: "step_update", step_update: { step_type: "agent_response", text_delta: "done-work", state: "DONE" } },
  { event: "result", result: {
      status: "FAILED",
...
`,
      ...
      const runtime = makeRuntime(env);
      const { message } = await collectStream(
        streamAgy(runtime, makeModel("e2e-model"), userContext("late"), {
          sessionId: "sess-late",
          timeoutMs: 5_000,
        }),
      );
      assert.equal(message.stopReason, "stop");
      assert.equal(textOf(message), "done-work");
